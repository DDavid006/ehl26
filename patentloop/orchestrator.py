...
class PatentLoop:
    def __init__(
        ...
        *,
        ...
        if getattr(self.llm, "staff_board", None) is None:
            self.llm.staff_board = StaffBoard(self.run_dir)
        self.staff_board = self.llm.staff_board
    ...
    def run(self, idea_text: str, *, max_iterations: int = MAX_ITERATIONS) -> dict:
        ...
        for iteration in range(1, max_iterations + 1):
            ...
            extracted, extract_log = extract_idea(
                current, self.llm, iteration=iteration
            )
            self.writer.event(
            ...
            feasibility, feasibility_logs = FeasibilityAgent(self.llm).run(
                current,
                extracted,
                iteration=iteration,
            )
            ...
            if (
                ...
                draft = DraftingAgent(self.llm).run(
                    current,
                    extracted,
                    research,
                    patents,
