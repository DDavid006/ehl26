"""Element-by-patent coverage matrix: which patents cover which idea elements.

Pure functions only — no LLM calls, no external APIs. ``uncovered`` is the key
output: the element ids that no retrieved patent covers, i.e. where the idea's
potential novelty lives.
"""
...
STOPWORDS = frozenset(
    {
        ...
        "into", "of", "on", "or", "the", "that", "to", "using", "with",
        "without",
    }
)
...
def is_covered(element: dict, patent: dict) -> tuple[bool, str]:
    ...
    when there is no match. Kept as its own function so the matching logic can
    be swapped out later (e.g. for embedding similarity).
    """
...
def build_matrix(elements: list[dict], patents: list[dict]) -> dict[str, Any]:
    ...
    Returns::

        {
          "elements": [...],
          "patents": [...],
          "coverage": {"E1": {"US1234567": {"covered": true, "evidence": "..."}}},
          "uncovered": ["E4"],
        }
...
def elements_from_extraction(extraction: dict[str, Any]) -> list[dict]:
    """Adapt a PatentLoop extraction (elements + search_queries) to matrix elements."""
    elements = extraction.get("elements") or []
    queries = extraction.get("search_queries") or []
    adapted = []
    for index, text in enumerate(elements):
        terms = [text]
        if index < len(queries) and queries[index] != text:
            terms.append(queries[index])
        adapted.append({"id": f"E{index + 1}", "text": text, "search_terms": terms})
    return adapted
